...
contract StakingTest is Test, Deployment
    ...
    assertEq(staking.userBalanceXSALT(alice), 5 ether);
    ...
    assertEq(staking.userBalanceXSALT(bob), 10 ether);
    ...
    assertEq(staking.userBalanceXSALT(charlie), 20 ether);
    ...
    assertEq(staking.userBalanceXSALT(alice), 8 ether);
	...
	uint256 userFreeXSALT = staking.userBalanceXSALT(alice);
	assertEq(userFreeXSALT, stakeAmount - unstakeAmount);

	uint256 totalStaked = staking.totalStakedOnPlatform();
	...
	staking.stakeSALT(10 ether);
	assertEq(staking.userBalanceXSALT(alice), 10 ether);
	assertEq(staking.totalStakedOnPlatform(), 10 ether);

	// Alice creates an unstake request with 5 ether for 3 weeks
	uint256 unstakeID = staking.unstake(5 ether, 3);
	assertEq(staking.userBalanceXSALT(alice), 5 ether);
	assertEq(staking.totalStakedOnPlatform(), 5 ether);
	...
	assertEq(staking.userBalanceXSALT(alice), 0 ether);
	...
	assertEq(staking.userBalanceXSALT(alice), initialStake);
	...
	uint256 xsaltBalance = staking.userBalanceXSALT(alice);
	for(uint256 i = 0; i < poolIDsToVote.length; i++) {
		...
		assertEq(staking.userBalanceXSALT(alice), xsaltBalance - voteAmounts[i]);
		...
		xsaltBalance = staking.userBalanceXSALT(alice);
	}

	// After voting for both poolIDs, Alice should have no free xSALT left
	assertEq(staking.userBalanceXSALT(alice), 0);
    }
	...
	function testUserRemovesVotesAndClaimsRewards() public {
        ...
        assertEq(staking.userBalanceXSALT(alice), 3 ether);
	...
	// A unit test which tests the userBalanceXSALT function for various users and checks that the returned freeXSALT balance is accurate.
	function testUserBalanceXSALT2() public {
        ...
        assertEq(staking.userBalanceXSALT(bob), 5 ether);
        ...
        assertEq(staking.userBalanceXSALT(charlie), 10 ether);
    }
	...
	function testSimultaneousStaking() public {
        ...
        assertEq(staking.userBalanceXSALT(alice), aliceStakeAmount);
        assertEq(staking.userBalanceXSALT(bob), bobStakeAmount);
        assertEq(staking.userBalanceXSALT(charlie), charlieStakeAmount);
	...
	function testDepositAndRemoveVotes() public {
        ...
        assertEq(staking.userBalanceXSALT(bob), 25 ether);
        ...
        assertEq(staking.userBalanceXSALT(alice), 40 ether);
        ...
        assertEq(staking.userBalanceXSALT(bob), 30 ether);
        assertEq(totalStakedForPool(poolIDs[1]), 20 ether);
        ...
        assertEq(staking.userBalanceXSALT(charlie), 40 ether);
	...
	function testStakeNegativeAmount() public {
        uint256 initialBalance = staking.userBalanceXSALT(alice);
        uint256 amountToStake = uint256(int256(-1));

        vm.expectRevert("Insufficient SALT balance to stake");
        vm.prank(alice);
        staking.stakeSALT(amountToStake);

        // Assert that Alice's balance remains unchanged
        assertEq(staking.userBalanceXSALT(alice), initialBalance);
    }
	...
	function testCancelUnstake2() public {
        ...
        vm.prank(alice);
        staking.stakeSALT(amountToStake);
        assertEq(staking.userBalanceXSALT(alice), amountToStake);
	...
	// A unit test which tests multiple users trying to recover each other's SALT tokens after unstaking and checks that only the original staker can recover the tokens.
	 function testRecoverSalt() public {
            ...
            assertEq(staking.userBalanceXSALT(alice), 50 ether);
            assertEq(staking.userBalanceXSALT(bob), 50 ether);
            assertEq(staking.userBalanceXSALT(charlie), 50 ether);
