...
// Keeps track of the arbitrage profits generated by pools (for proportional rewards distribution).
contract PoolStats is IPoolStats
	...
	function _accumulateProfit( IERC20 tokenA, IERC20 tokenB, uint256 arbitrageProfit, uint256[] memory _profits ) internal view
		...
		_profits[ _poolIndicies[poolID] ] += arbitrageProfit;
		}
	...
	function _calculateArbitrageProfits( bytes32[] memory poolIDs, uint256[] memory _calculatedProfits ) public view
		...
		for( uint256 i = 0; i < poolIDs.length; i++ )
			...
			if ( poolsConfig.isWhitelisted(poolID) )
				...
				_accumulateProfit( weth, tokenA, arbitrageProfit, _calculatedProfits );
				_accumulateProfit( tokenA, tokenB, arbitrageProfit, _calculatedProfits );
				_accumulateProfit( tokenB, weth, arbitrageProfit, _calculatedProfits );
				}
			else
				{
				...
				_accumulateProfit( weth, tokenA, arbitrageProfit, _calculatedProfits );
				_accumulateProfit( tokenA, wbtc, arbitrageProfit, _calculatedProfits );
				_accumulateProfit( wbtc, tokenB, arbitrageProfit, _calculatedProfits );
				_accumulateProfit( tokenB, weth, arbitrageProfit, _calculatedProfits );
				}
			}
		}
