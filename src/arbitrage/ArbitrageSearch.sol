...
contract ArbitrageSearch
	...
	function _determineProfitsWhitelisted( uint256 midpoint, uint256 reservesA0, uint256 reservesA1, uint256 reservesB0, uint256 reservesB1, uint256 reservesC0, uint256 reservesC1, uint256 kA, uint256 kB ) internal pure returns (int256 profitMidpoint, int256 profitRightOfMidpoint )
		{
		// Would have been nice to pass this as an arg, but ran out of stack
	...
	function _binarySearchWhitelisted( uint256 swapAmountInValueInETH, uint256 reservesA0, uint256 reservesA1, uint256 reservesB0, uint256 reservesB1, uint256 reservesC0, uint256 reservesC1 ) internal pure returns (uint256 bestArbAmountIn)
		{
	...
	// Given the reserves for the arbitrage swap, calculate the profit at the midpoint of the current possible range and just to the right of the midpoint.
	function _determineProfitsNonWhitelisted( uint256 midpoint, uint256 reservesA0, uint256 reservesA1, uint256 reservesB0, uint256 reservesB1, uint256 reservesC0, uint256 reservesC1, uint256 reservesD0, uint256 reservesD1 ) internal pure returns (int256 profitMidpoint, int256 profitRightOfMidpoint )
		{
		uint256 kA = reservesA0 * reservesA1;
		uint256 kB = reservesB0 * reservesB1;
		uint256 kC = reservesC0 * reservesC1;
	...
	// The search will be done using a binary search algorithm where profits are determined at the midpoint of the current range, and also just to the right of the midpoint.
	// Assuming that the profit function is unimodal (which may not actually be true), the two profit calculations at and near the midpoint can show us which half of the range the maximum profit is in.
	function _binarySearchNonWhitelisted( uint256 swapAmountInValueInETH, uint256 reservesA0, uint256 reservesA1, uint256 reservesB0, uint256 reservesB1, uint256 reservesC0, uint256 reservesC1, uint256 reservesD0, uint256 reservesD1 ) internal pure returns (uint256 bestArbAmountIn)
		{
		if ( reservesA0 <= PoolUtils.DUST || reservesA1 <= PoolUtils.DUST || reservesB0 <= PoolUtils.DUST || reservesB1 <= PoolUtils.DUST || reservesC0 <= PoolUtils.DUST || reservesC1 <= PoolUtils.DUST || reservesD0 <= PoolUtils.DUST || reservesD1 <= PoolUtils.DUST )
			return 0;
		...
		for( uint256 i = 0; i < 5; i++ )
			{
			(profitMidpoint, profitRightOfMidpoint) = _determineProfitsNonWhitelisted( (leftPoint + rightPoint) >> 1, reservesA0, reservesA1, reservesB0, reservesB1, reservesC0, reservesC1, reservesD0, reservesD1 );
