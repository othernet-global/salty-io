...
contract TestUpkeep2 is Deployment
    ...
    function testSuccessStep6() public
    	...
    	assertEq( salt.balanceOf(address(stakingRewardsEmitter)), 3000000000000000000000000 );  // 3 million
    	assertEq( salt.balanceOf(address(liquidityRewardsEmitter)), 5000000000000000000000000 ); // 5 million
		...
		// 3 million emitting at 1% per day for one hour = 1250
		assertEq( staking.totalRewardsForPools(poolIDsA)[0], uint256(3000000 ether) / 100 / 24 );
		...
		// Check if the rewards were transferred (5 million / 8 pools emitting at 1% per day for one hour) to the liquidity contract
		uint256[] memory rewards = liquidity.totalRewardsForPools(poolIDs);

		assertEq( rewards[0], uint256(5000000 ether) / numInitialPools / 100 / 24);
		assertEq( rewards[1], uint256(5000000 ether) / numInitialPools / 100 / 24);
		assertEq( rewards[2], uint256(5000000 ether) / numInitialPools / 100 / 24);
	  	}
	...
	function _secondPerformUpkeep() internal
		...
		// Check Step 1. Withdraws deposited SALT arbitrage profits from the Pools contract and rewards the caller of performUpkeep()
		// From the directly added 100 ether + arbitrage profits
    	assertEq( salt.balanceOf(upkeepCaller), 10009966589553192391 );


		// Check Step 2. Burns 10% of the remaining withdrawn salt and sends 10% to the DAO's reserve.

		assertEq( salt.totalBurned(), 19018936520151065544 );
		...
		// One hour of emitted rewards (1% a day) of (3 million bootstrapping in the stakingRewardsEmitter and 50% of 1560.2 ether)
		// Keep in mind that totalRewards contains rewards that have already been claimed (and become virtual rewards)
		assertEq( staking.totalRewardsForPools(poolIDsA)[0], 1354493270797834104251  );
		...
		assertEq( rewards[0], 282232348946500256972);
		assertEq( rewards[1], 282232348946500256972);
		assertEq( rewards[2], 282232348946500256972);
		...
		// The daoVestingWallet contains 25 million SALT and vests over a 10 year period.
		// 100k SALT were removed from it in _generateArbitrageProfits() - so it emits about 34394 in the first 5 days + one hour
		assertEq( salt.balanceOf(address(dao)), 34436620724952418949866 );
		}
