...
// It is done by waiting for users to swap in the opposiite direction and then swapping - essentially unoding the original user swap.
// This is done to gradually swap WETH from arbitrage profits to tokens for Protocol Owned Liquidity and liquidated WBTC/WETH liquidity to USDS so that it can be burned.
// Counterswap deposits to the Pools contract are owned by constant addresses below.

library Counterswap
	{
	// Counterswap addresses
	...
	function _determineCounterswapAddress( IERC20 tokenToCounterswap, IERC20 desiredToken, IERC20 wbtc, IERC20 weth, IERC20 salt, IERC20 usds ) internal view returns (address counterswapAddress)
		...
		if ( address(tokenToCounterswap) == address(wbtc) )
		if ( address(desiredToken) == address(usds) )
			return WBTC_TO_USDS;
